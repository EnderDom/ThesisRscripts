...
dig_clc <- read.table("data/ClcBio_Digestive.dat", header=T, sep=" ");
dig_new <- read.table("data/Newbler_Digestive.dat", header=T, sep=" ");
dig_seq <- read.table("data/Seqman_Digestive.dat", header=T, sep=" ");
...
plot(eCLC, dig_clc$HITS, type="n",font.main = 1, main="Number of contigs with hits vs nr at expect values for digestive tissue", xlab="Evalue (1e^x)", ylab="Number of Top Hits below E-value")
